...
public class FinalExplosion extends Explosion{
	...
	public void draw(Main main) {
		if ((main.getFrameCount()-getFrames())<30){			
			...
			(int)(0.1*Math.sqrt(getFrames()/30.0*(main.getFrameCount()-getFrames())/30.0)*getSize()),
			(int)(0.1*Math.sqrt(getFrames()/30.0*(main.getFrameCount()-getFrames())/30.0)*getSize()));
		}
		else {
			main.remove(this);
			...
			main.getMessage()[13]="Deine Trefferquote: "+
								100.0*(Main.getStatistic().getShots()-Main.getStatistic().getMissed())/Main.getStatistic().getShots()+"%";
			main.getMessage()[14]="";
			main.getMessage()[15]="------------------------------------------------";
			main.getMessage()[16]="";
			main.getMessage()[17]="Punktzahl: "+main.getScore()+" * "+1.0*(Main.getStatistic().getShots()-Main.getStatistic().getMissed())/Main.getStatistic().getShots()
								+" = "+((int)(1.0*main.getScore()*(Main.getStatistic().getShots()-Main.getStatistic().getMissed())/Main.getStatistic().getShots()));
			main.getMessage()[18]="";
